...
def _make_host():
    ...
    host.device_pool = SimpleNamespace(
        ...
        k_scale_buffer=[
            torch.zeros(PAGE_NUM * PAGE_SIZE, HEAD_NUM) for _ in range(LAYER_NUM)
        ],
        v_scale_buffer=[
            torch.zeros(PAGE_NUM * PAGE_SIZE, HEAD_NUM) for _ in range(LAYER_NUM)
        ],
...
class TestDynamicFP8MHATokenToKVPoolHost(CustomTestCase):
    ...
    def test_scale_rows_round_trip_through_l2(self):
        host = _make_host()
        source = torch.arange(
            LAYER_NUM * PAGE_NUM * PAGE_SIZE * HEAD_NUM, dtype=torch.float32
        ).reshape(LAYER_NUM, PAGE_NUM * PAGE_SIZE, HEAD_NUM)
