...
class TestQsaCompressedWritePlan(CustomTestCase):
    ...
    def test_hicache_pages_alias_compressed_qsa_storage(self):
        from sglang.srt.mem_cache.qsa_kv_pool import QSATokenToKVPool

        pool = QSATokenToKVPool.__new__(QSATokenToKVPool)
        pool.qsa_compressed_page_size = 16
        pool.qsa_index_kv_heads = 1
        pool.qsa_index_head_dim = 2
        pool.qsa_compressed_k_buffer_pool = [
            torch.zeros((32, 1, 2), dtype=torch.bfloat16)
        ]
        source_bytes = torch.arange(128, dtype=torch.uint8).reshape(32, 1, 4)
        pool.qsa_compressed_k_buffer_pool[0].view(torch.uint8).copy_(source_bytes)

        pages = pool.get_hicache_indexer_page_buffers()

        self.assertEqual(tuple(pages[0].shape), (2, 64))
        self.assertEqual(
            pages[0].data_ptr(), pool.qsa_compressed_k_buffer_pool[0].data_ptr()
        )
        self.assertTrue(torch.equal(pages[0].reshape(32, 1, 4), source_bytes))

    def test_compressed_qsa_read_waits_for_layer_restore(self):
        from sglang.srt.mem_cache.qsa_kv_pool import QSATokenToKVPool

        pool = QSATokenToKVPool.__new__(QSATokenToKVPool)
        pool.start_layer = 0
        pool.full_attention_layer_id_mapping = {7: 0}
        pool.layer_transfer_counter = mock.Mock()
        expected = torch.zeros((16, 1, 2), dtype=torch.bfloat16)
        pool.qsa_compressed_k_buffer_pool = [expected]

        actual = pool.get_qsa_compressed_k_buffer(7)
